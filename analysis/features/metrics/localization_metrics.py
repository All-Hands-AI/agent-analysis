...
from analysis.models.localization import Location, ScopeKind
...
class LocalizationMetrics(BaseModel):
    ...
    @staticmethod
    def localization_of_kind(
        generated_locations: list[Location],
        gold_locations: list[Location],
        kind: ScopeKind,
        ...
        gold_scopes = set(
            [
                ...
                if (scope := loc.most_recent_scope(kind)) is not None
            ]
        )
        generated_scopes = set(
            [
                scope
        ...
        precision = (
            len(gold_scopes & generated_scopes) / len(generated_scopes)
            if generated_scopes
            else None
        )
    ...
    @staticmethod
    def from_locations(
        generated_locations: list[Location], gold_locations: list[Location]
    ) -> LocalizationMetrics:
        """
        Compute localization metrics for a list of locations.

        Args:
            generated_locations: The locations generated by the model.
            gold_locations: The golden locations acting as ground truth.
        """
        # File statistics
        file_match, file_precision = LocalizationMetrics.localization_of_kind(
            generated_locations, gold_locations, ScopeKind.FILE
        )

        # Function statistics
        function_match, function_precision = LocalizationMetrics.localization_of_kind(
            generated_locations, gold_locations, ScopeKind.FUNCTION
        )

        # Class statistics
        class_match, class_precision = LocalizationMetrics.localization_of_kind(
            generated_locations, gold_locations, ScopeKind.CLASS
        )
    ...
    def from_patch(generated_patch: Patch, gold_patch: Patch) -> LocalizationMetrics:
        ...
        return LocalizationMetrics.from_locations(generated_locations, gold_locations)
